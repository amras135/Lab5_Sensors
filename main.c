...
void SysTick_Handler(void) {
  ...
  sec++;
	...
	LCD_DisplayString(dig);
}

uint32_t SystemCoreClock    = 16000000;

void BarDisplay(uint8_t);
...
int main(void){
 ...
 BarDisplay(distance);
	...
	while(1) {
		Check_Color(&leftSensor);
		ReflectanceDisplay(display, bar[0], bar[1]);
	}
		
	
}

void BarDisplay(uint8_t distance)
{
